...
import { getCookie, setCookie } from '../common/cookieUtils';
...
function LoginForm() {
    ...
    //그럼 로그인 창에 쿠키가 떨어졌을 때 바로 백엔드 /auth/signin엔드 포인트에 포스트

    const checkCookie = getCookie('test');

    useLayoutEffect(() => {
        if (checkCookie) {
            console.log(checkCookie);// 백엔드에 api쏠 수 있도록 설정
        }
    }, [checkCookie]); // emailCookie만 의존성으로 설정
